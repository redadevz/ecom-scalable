            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <section :class="cardCls">
                    <header :class="headerCls">
                        <span :class="iconBadge"><LanguageIcon class="h-5 w-5" /></span>
                        <div>
                            <h3 :class="titleCls">Details</h3>
                            <p :class="subCls">The language name, locale code and description.</p>
                        </div>
                    </header>
                ...
                <div class="lg:sticky lg:top-24 space-y-6">
                    ...
                    <section class="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-[#2c2f3d] dark:bg-[#1e2029]">
                        <div class="relative h-28 bg-gradient-to-br from-primary-500 via-orange-500 to-amber-400">
                            <GlobeAltIcon class="absolute -right-4 -top-4 h-32 w-32 text-white/15" />
                        ...
                        <div class="px-5 pb-5">
                            ...
                            <p class="text-sm text-gray-400">{{ form.short_name || '—' }}</p>
                            ...
                            <dl class="mt-4 space-y-2 border-t border-gray-100 pt-4 text-sm dark:border-[#2a2d38]">
                                <div v-if="form.description" class="flex items-start gap-2 text-gray-600 dark:text-gray-300">
                                    <DocumentTextIcon class="mt-0.5 h-4 w-4 flex-shrink-0 text-gray-400" /><span>{{ form.description }}</span>
                                </div>
                                <p v-else class="text-xs text-gray-400">A description of the language appears here as you fill it in.</p>
                            </dl>
                        </div>
                    </section>
                </div>
...
const displayName = computed(() => props.form.name || "New Language");

const initials = computed(() => {
    const base = (props.form.name || "").toString().trim();
    const parts = base.split(/\s+/).filter(Boolean);
    if (!parts.length) return (props.form.short_name || "L").toString().slice(0, 2).toUpperCase();
