<template>
    <Wizard :steps="steps" :processing="form.processing" :errors="form.errors" :field-step="fieldStep"
        submit-label="Save supplier" max-width="3xl" @submit="submit">
        ...
        <template #settings>
            <div class="space-y-6 p-6 sm:p-8">
                ...
                <div class="divide-y divide-gray-100 rounded-xl border border-gray-200 dark:divide-[#2a2d38] dark:border-[#2c2f3d]">
                    <Toggle v-model="form.is_active" title="Active" desc="Inactive suppliers are hidden from operations." />
                    <Toggle v-model="form.is_tax_exempted" title="Tax exempted" desc="No tax applied to this supplier." />
                </div>
...
const fieldStep: Record<string, number> = {
    ...
    store_id: 2, created_by: 2, is_active: 2, is_tax_exempted: 2, comments: 2,
};

const displayName = computed(() => {
    if (props.form.is_company && props.form.company_name) return props.form.company_name;
    return [props.form.first_name, props.form.last_name].filter(Boolean).join(" ").trim() || "New supplier";
});
