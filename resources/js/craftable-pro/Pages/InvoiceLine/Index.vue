    ...
    <PageContent>
        ...
        >
            ...
            <template #tableRow="{ item, action }: any">
                <!-- Identity: initials avatar + line no + invoice ref -->
                <ListingDataCell>
                    <div class="flex items-center gap-3">
                        <span class="flex h-11 w-11 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg bg-primary-500/10 text-sm font-bold uppercase text-primary-600 dark:text-primary-400">
                            {{ String(item.line_no ?? '?').slice(0, 2) }}
                        </span>
                        <div class="flex flex-col leading-tight">
                            <span class="font-medium text-gray-900 dark:text-white">{{ $t("craftable-pro", "Line") }} {{ item.line_no }}</span>
                            <span class="text-xs text-gray-400">{{ item.invoice?.invoice_no ? '#' + item.invoice.invoice_no : '—' }}</span>
                        </div>
                    </div>
                </ListingDataCell>
                ...
                <!-- Invoice ref pill -->
                <ListingDataCell>
                    <span v-if="item.invoice?.invoice_no"
                ...
                <!-- Order line ref -->
                <ListingDataCell>
                    <span class="text-sm text-gray-700 dark:text-gray-200">{{ item.order_line?.line_no ?? '—' }}</span>
                </ListingDataCell>
                ...
                <ListingDataCell>
                    <div class="flex items-center justify-center gap-2">
                        <Link :href="route('craftable-pro.invoice-lines.edit', item)" title="View"
                        ...
                        <Link :href="route('craftable-pro.invoice-lines.edit', item)" title="Edit" v-can="'craftable-pro.invoice-lines.edit'"
                        ...
                        <Modal type="danger">
                            <template #trigger="{ setIsOpen }">
                                <button @click="() => setIsOpen(true)" title="Delete" v-can="'craftable-pro.invoice-lines.destroy'"
                            ...
                            <template #title>{{ $t("craftable-pro", "Delete Invoice Line") }}</template>
                            <template #content>
                                {{ $t("craftable-pro", "Are you sure you want to delete selected Invoice Line? All data will be permanently removed from our servers forever. This action cannot be undone.") }}
                            </template>
                            <template #buttons="{ setIsOpen }">
                                <Button @click.prevent="() => { action('delete', route('craftable-pro.invoice-lines.destroy', item), { onFinish: () => setIsOpen(false) }); }" color="danger" v-can="'craftable-pro.invoice-lines.destroy'">
                                    {{ $t("craftable-pro", "Delete") }}
                                </Button>
